...
vi.mock("../../utils/patterns", () => ({
	getEffectivePatterns: vi.fn(
		(_moduleId: string, patterns: string[]) => patterns,
	),
}));
...
const { getGitHubToken, createPullRequest } = await import(
	"../../utils/github"
);
const {
	confirmAction,
...
describe("pushCommand", () => {
	...
	describe("run", () => {
		...
		it("--files オプションで指定ファイルのみ PR に含める", async () => {
			...
			expect(
				callArgs.files.some(
					(f: any) => f.path === ".devcontainer/devcontainer.json",
				),
			).toBe(false);
		});
		...
		it("--files に一致するファイルがない場合はキャンセル", async () => {
			...
			expect(mockLog.info).toHaveBeenCalledWith(
				"No matching files found. Cancelled.",
			);
			expect(mockCreatePullRequest).not.toHaveBeenCalled();
		});
		...
		it("baseHashes が存在しコンフリクトがある場合は警告して確認を求める（baseRef なし）", async () => {
			...
			expect(mockLog.warn).toHaveBeenCalledWith(
				expect.stringContaining("Template updated"),
			);
			expect(mockLog.info).toHaveBeenCalledWith(
...
describe("pushCommand args", () => {
	it("args に yes フラグが定義されている", () => {
		const args = pushCommand.args as Record<
			string,
