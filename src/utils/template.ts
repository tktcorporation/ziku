 ...
 */
export function buildTemplateSource(source: {
	owner: string;
	repo: string;
	ref?: string;
}): string {
...
export async function downloadTemplateToTemp(
	...
	const tempDir = join(
		targetDir,
		label ? `.devenv-temp-${label}` : ".devenv-temp",
	);

	ensureGigetCacheDir();
	const { dir: templateDir } = await downloadTemplate(
...
export async function fetchTemplates(
	...
	try {
		...
		for (const moduleId of modules) {
			...
			const { tracked, ignored } = separateByGitignore(
				resolvedFiles,
				gitignore,
			);
