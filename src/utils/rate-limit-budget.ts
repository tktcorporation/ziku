...
/**
 * 観測残量から、まだレスポンスが返っていない（＝観測残量にまだ反映されていない）リクエストの
 * 予約分を差し引いた実効残量を求める。
 *
 * `--since` 指定時のコミット日時取得は、複数の候補（利用リポジトリ）が並行して各々の変更
 * ファイルへ動的ブレーキ（{@link cannotAffordRemainingRequests}）を判定する。各候補が
 * 「自分の担当ファイル配列内の残数」だけを見ると、他の候補が同時に発行しようとしている分
 * （まだレスポンスが返らず観測残量に反映されていない分）を考慮できず、複数の候補が同じ
 * 観測残量を互いに知らないまま「自分は使ってよい」と判定してしまう。呼び出し側
 * （`aggregate.ts`）がプロセス全体で共有する予約カウンタを持ち、リクエストを実際に発行する
 * 前にこの関数へ現在の予約数を渡すことで、動的ブレーキの入力を「観測済みの残量」から
 * 「今まさに使われようとしている分」を引いた、より保守的な値に置き換えられる。
 *
 * `reserved` が `observedRemaining` を上回っても結果が負になることはない。
 */
export function effectiveRemaining(observedRemaining: number, reserved: number): number {
  return Math.max(0, observedRemaining - reserved);
}
