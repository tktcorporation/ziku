 ...
 * 6. テンプレートの正規名解決（lock.json の `source`（owner/repo）が、テンプレートがリネーム・
 *    移管された後の現在の呼び出し引数と文字列上一致しない場合のみ、旧名を正規名へ解決する
 *    ために追加で 1 リクエストを発行する。この解決は owner/repo の組ごとにキャッシュされる
 *    ため、同じ旧名を持つ候補が複数あっても 1 回で済むが、候補ごとに異なる旧名を持つ場合は
 *    候補の数だけ発生しうる。テンプレートがリネームされておらず lock.json が現在の名前を
 *    指している利用リポジトリでは発生しない）
 *
 * 1〜4 は必ず発生し 5 リクエスト、5・6 はそれぞれ最悪ケースで 2・1 リクエスト加わり、
 * 合計で最悪 8。
 *
 * `since` フィルタ指定時のコミット日時取得など、これを超える呼び出しが発生するケースも
 * あるため、あくまで下限の見積もりであることに注意。
 */
export const ESTIMATED_REQUESTS_PER_CANDIDATE = 8;
