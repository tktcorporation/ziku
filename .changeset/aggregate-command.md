...
- 候補リポジトリごとに commit SHA を先に解決し、`.ziku/lock.json` の取得とリポジトリ内容のダウンロードを同じ SHA に固定する。スキャン中に対象リポジトリで `ziku pull` が並行して走っても、新しいファイルと古い `baseHashes` を突き合わせて実在しない conflict/pending を報告することがない。
