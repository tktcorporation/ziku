...
4. `excludePatterns` が consumer 側 (`.ziku.json`) にしかなく、upstream から除外宣言できない
...
- **`.ziku/modules.jsonc` がパターン定義**、**`.ziku.json` が同期状態** — 責務を分離する
- **`.ziku.json` から `modules` 配列と `excludePatterns` を削除する**
...
| `.ziku.json` | 同期状態 (source, baseRef, baseHashes, pendingMerge) | consumer プロジェクト |
...
11. **`src/commands/init.ts`**
    ...
    - `.ziku.json` 保存時に `modules` フィールドを含めない
...
- `name` のリネームは modules.jsonc だけの変更で済む (.ziku.json に影響しない)
