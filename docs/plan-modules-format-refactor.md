...
4. `excludePatterns` が consumer 側 (`.ziku/config.json`) にしかなく、upstream から除外宣言できない
...
- **`.ziku/modules.jsonc` がパターン定義**、**`.ziku/config.json` が同期状態** — 責務を分離する
- **`.ziku/config.json` から `modules` 配列と `excludePatterns` を削除する**
...
| `.ziku/config.json`                   | 同期状態 (source, baseRef, baseHashes, pendingMerge) | consumer プロジェクト  |
...
11. **`src/commands/init.ts`**
    ...
    - `.ziku/config.json` 保存時に `modules` フィールドを含めない
...
- `name` のリネームは modules.jsonc だけの変更で済む (.ziku/config.json に影響しない)
